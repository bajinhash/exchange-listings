...
10. **日期过滤（关键规则 / 24h 滚动窗口）**：报告日期 X = 上海时间（CST，UTC+8）。窗口 = **昨天 18:00 CST → 今天 18:00 CST**（24 小时滚动）。
    具体保留规则（基于**公告发布时间**，不是上线/开盘时间）：
    - 公告发布在窗口内（X-1 当天 18:00 之后 到 X 当天 18:00 之前）→ 保留
    - 公告发布在更早 → **直接丢弃**，归入周回顾
    - 公告发布晚于 X 18:00 → 等明天的报告再说
    - 注意：listing 的实际开盘时间可以在更晚（例如"将于明天上线"），只要**公告本身**发在窗口内就保留
    - 判断公告发布时间的优先级：body 中 "Published: ISO时间戳" > body 中 "Published: YYYY-MM-DD" > 标题/body 里其它日期
